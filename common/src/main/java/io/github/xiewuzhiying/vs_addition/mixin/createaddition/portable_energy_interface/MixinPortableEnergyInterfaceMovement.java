 ...
 * 4. 使用字符串比较替代直接类型引用，避免缺少 CCA 时的崩溃
 *
 * 关键假设：
...
public abstract class MixinPortableEnergyInterfaceMovement {
    ...
    private boolean vs_addition$findStationaryInterface(
        ...
        // 使用字符串比较检查是否为 CreateAdditions 的能量接口
        // 避免直接引用类，防止缺少 CCA 时崩溃
        if (instance.getClass().getName().equals("com.mrh0.createaddition.blocks.portable_energy_interface.PortableEnergyInterfaceBlockEntity")) {
            // 检查是否启用了飞船行为的特殊模式
